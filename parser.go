...
func NewParser(text []byte) *parser {
	...
	lexer := NewLexer(text)
	p := &parser{pos: 0, data: ex, symbols: symbols, lexer: lexer}
...
func (self *parser) tok(offset int) Token {
	return self.tokens[self.pos+offset%4]
}

func (self *parser) advance(offset int) {
	for i := 0; i < offset; i++ {
		t, err := self.lexer.Lex()
		if err != nil {
			panic(err)
		}
		self.tokens[(self.pos+i)%4] = t
	}
	self.pos = (self.pos + offset) % 4
}
